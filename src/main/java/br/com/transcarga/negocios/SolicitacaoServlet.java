...
public class SolicitacaoServlet extends HttpServlet {
    ...
    private void mostrarLista(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        List<Frete> aguardandoAdmin = solicitacoes.stream()
                .filter(f -> f.getDataRespostaAdmin() == null)
                .collect(Collectors.toList());
        List<Frete> aguardandoUser = solicitacoes.stream()
                .filter(f -> f.getDataRespostaAdmin() != null && !"Rejeitado".equals(f.getStatus()))
                .collect(Collectors.toList());
        List<Frete> rejeitadas = solicitacoes.stream()
                .filter(f -> "Rejeitado".equals(f.getStatus()))
                .collect(Collectors.toList());
        ...
        boolean temAlguma = !aguardandoAdmin.isEmpty() || !aguardandoUser.isEmpty() || !rejeitadas.isEmpty();
        ...
        if (!temAlguma) {
            out.println("<div class='empty-message'><p>Nenhuma solicitação encontrada.</p></div>");
        }
